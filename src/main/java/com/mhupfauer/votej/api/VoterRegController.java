...
public class VoterRegController {
    ...
    @PostMapping("/")
    public ResponseEntity<HttpResponse> newRegisteredVoter(@PathVariable(name = "event_id") Long eid, @PathVariable(name = "ballot_id") Long bid, @RequestBody Map<String, Object> fields) {
        ...
        if(fields.containsKey("user") &&
                (helper.getCurrentUser().getRole() != ROLES.ADMIN
                        || !helper.getCurrentUser().equals(eventEntRepository.findById(eid).get().getOrganizer())
                )
        ...
        if(voterRegEntRepository.getVoterRegEntByUserAndBallot(helper.getCurrentUser(),
                ballotEntRepository.findById(bid).get()) != null) {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
        }
    ...
    public ResponseEntity<HttpResponse> editRegisteredVoter(@PathVariable(name = "ballot_id") Long bid, @PathVariable("voter_reg_id") Long vid, @RequestBody Map<String, Object> fields) {
        if (ballotEntRepository.findById(bid).isEmpty() || voterRegEntRepository.findById(vid).isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
        helper.changeField(fields, "ballot", bid);
    ...
    public ResponseEntity<HttpResponse> deleteRegisteredVoter(@PathVariable(name = "voter_reg_id") Long vid) {
        if (voterRegEntRepository.findById(vid).isEmpty()) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        }
