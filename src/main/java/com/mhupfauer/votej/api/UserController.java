...
public class UserController {
  ...
  public ResponseEntity<HttpStatus> patchUserAdmin(
      @PathVariable(name = "user_id") Long uid, @RequestBody Map<String, Object> fields) {
    if (userEntRepository.findById(uid).isEmpty())
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
    if (fields.containsKey("id")) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    if (fields.containsKey("email")
        && userEntRepository.findByEmail(fields.get("email").toString()) != null)
      return new ResponseEntity<>(HttpStatus.FORBIDDEN);
  ...
  public ResponseEntity<HttpStatus> patchUser(
      @PathVariable(name = "user_id") Long uid, @RequestBody Map<String, Object> fields) {
    UserEnt currentUser =
        ((AuthenticatedUser) SecurityContextHolder.getContext().getAuthentication().getPrincipal())
            .getUser();

    if (currentUser.getId().equals(uid)) return new ResponseEntity<>(HttpStatus.FORBIDDEN);
