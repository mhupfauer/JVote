...
public class EventController {
    ...
    public Object getEventById(@PathVariable(name = "id") Long id) {
        if (eventEntRepository.findById(id).isEmpty()) return new ResponseEntity<HttpStatus>(HttpStatus.NOT_FOUND);
    ...
    public ResponseEntity<HttpStatus> newEvent(@RequestBody Map<String, Object> fields) {
        ...
        if (helper.setFields(fields, eventToCreate)) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
    ...
    public ResponseEntity<HttpStatus> patchEvent(@PathVariable(name = "id") Long id, @RequestBody Map<String, Object> fields) {
        if (eventEntRepository.findById(id).isEmpty()) return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        var event = eventEntRepository.findById(id).get();
        if (helper.setFields(fields, event)) return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
