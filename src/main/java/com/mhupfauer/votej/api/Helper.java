...
public class Helper {
  ...
  @Autowired private AnswerEntRepository answerEntRepository;
    ...
    Arrays.asList(this.getClass().getDeclaredFields())
        .forEach(
            (fieldz ->
                map.put(
                    ((Class)
                        ((ParameterizedType) fieldz.getType().getGenericInterfaces()[0])
        ...
        (k, v) -> {
          ...
          if (this.getHandleableCustomTypes().containsKey(field.getType())) {
            ...
            Arrays.asList(clazz.getDeclaredMethods())
                .forEach(
                    (method) -> {
                      if (method.getName().toLowerCase().equals("set" + k)
          ...
          if (!Arrays.asList(String.class, Integer.class, Long.class, Boolean.class)
  ...
  public Map<String, Object> getMapWithoutFields(Object obj, String... fields) {
    ...
    List<Field> fieldList =
        Arrays.stream(obj.getClass().getDeclaredFields())
            .filter(field -> !fieldsToRemove.contains(field.getName()))
